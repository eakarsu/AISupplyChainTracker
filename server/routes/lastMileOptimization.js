...
// Last-mile delivery optimization with route and consolidation recommendations
// Feature: last-mile-optimization
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You are an expert assistant for the "AISupplyChainTracker" platform. Always return strict JSON only — no markdown, no commentary.',
      `Feature: Last-mile delivery optimization with route and consolidation recommendations.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'last-mile-optimization', payload, result);
    res.json({ success: true, feature: 'last-mile-optimization', ...result });
  } catch (err) {
    console.error('last-mile-optimization analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You analyze batches of items for "AISupplyChainTracker". Return strict JSON only.',
      `Feature: Last-mile delivery optimization with route and consolidation recommendations. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'last-mile-optimization', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: 'last-mile-optimization', title: 'Last-mile delivery optimization with route and consolidation recommendations', project: 'AISupplyChainTracker' });
});
