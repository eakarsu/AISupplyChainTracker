...
// IoT sensor stream ingestion for cold-chain compliance
// Feature: iot-cold-chain
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: IoT sensor stream ingestion for cold-chain compliance.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'iot-cold-chain', payload, result);
    res.json({ success: true, feature: 'iot-cold-chain', ...result });
  } catch (err) {
    console.error('iot-cold-chain analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: IoT sensor stream ingestion for cold-chain compliance. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'iot-cold-chain', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: 'iot-cold-chain', title: 'IoT sensor stream ingestion for cold-chain compliance', project: 'AISupplyChainTracker' });
});
