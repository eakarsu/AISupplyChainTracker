...
// Predictive quality issues flagging suppliers/routes with quality risk
// Feature: predictive-quality-issues
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Predictive quality issues flagging suppliers/routes with quality risk.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'predictive-quality-issues', payload, result);
    res.json({ success: true, feature: 'predictive-quality-issues', ...result });
  } catch (err) {
    console.error('predictive-quality-issues analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Predictive quality issues flagging suppliers/routes with quality risk. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'predictive-quality-issues', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: 'predictive-quality-issues', title: 'Predictive quality issues flagging suppliers/routes with quality risk', project: 'AISupplyChainTracker' });
});
