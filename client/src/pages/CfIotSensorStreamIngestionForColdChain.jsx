...
export default function CfIotSensorStreamIngestionForColdChain() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-iot-sensor-stream-ingestion-for-cold-chain-compliance/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-iot-sensor-stream-ingestion-for-cold-chain-compliance', project: 'AISupplyChainTracker' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-iot-sensor-stream-ingestion-for-cold-chain-compliance/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">IoT sensor stream ingestion for cold-chain compliance</h1>
