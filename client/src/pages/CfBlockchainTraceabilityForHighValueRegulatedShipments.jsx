...
export default function CfBlockchainTraceabilityForHighValueRegulatedShipments() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-blockchain-traceability-for-high-value-regulated-shipments/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-blockchain-traceability-for-high-value-regulated-shipments', project: 'AISupplyChainTracker' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-blockchain-traceability-for-high-value-regulated-shipments/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Blockchain traceability for high-value/regulated shipments</h1>
