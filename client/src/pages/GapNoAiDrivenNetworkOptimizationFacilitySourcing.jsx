...
export default function GapNoAiDrivenNetworkOptimizationFacilitySourcing() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-ai-driven-network-optimization-facility-sourcing-point-placement/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-ai-driven-network-optimization-facility-sourcing-point-placement', project: 'AISupplyChainTracker' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-ai-driven-network-optimization-facility-sourcing-point-placement/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No AI-driven network optimization (facility/sourcing point placement)</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_ai · AISupplyChainTracker</p>
      </div>
