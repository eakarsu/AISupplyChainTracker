...
export default function CfLastMileDeliveryOptimizationWithRouteAnd() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-last-mile-delivery-optimization-with-route-and-consolidation-recommendations/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-last-mile-delivery-optimization-with-route-and-consolidation-recommendations', project: 'AISupplyChainTracker' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-last-mile-delivery-optimization-with-route-and-consolidation-recommendations/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Last-mile delivery optimization with route and consolidation recommendations</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AISupplyChainTracker</p>
      </div>
