...
export default function GapNoIotSensorIngestionTemperatureHumidityFor() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-iot-sensor-ingestion-temperature-humidity-for-cold/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-iot-sensor-ingestion-temperature-humidity-for-cold', project: 'AISupplyChainTracker' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-iot-sensor-ingestion-temperature-humidity-for-cold/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No IoT sensor ingestion (temperature, humidity for cold chain)</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_non_ai · AISupplyChainTracker</p>
      </div>
