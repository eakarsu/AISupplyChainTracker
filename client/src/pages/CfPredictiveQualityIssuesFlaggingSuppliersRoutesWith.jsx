...
export default function CfPredictiveQualityIssuesFlaggingSuppliersRoutesWith() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-predictive-quality-issues-flagging-suppliers-routes-with-quality/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-predictive-quality-issues-flagging-suppliers-routes-with-quality', project: 'AISupplyChainTracker' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-predictive-quality-issues-flagging-suppliers-routes-with-quality/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Predictive quality issues flagging suppliers/routes with quality risk</h1>
