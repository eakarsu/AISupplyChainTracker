...
export default function CfNetworkOptimizationRecommendingFacilitySourcingLocations() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-network-optimization-recommending-facility-sourcing-locations/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-network-optimization-recommending-facility-sourcing-locations', project: 'AISupplyChainTracker' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-network-optimization-recommending-facility-sourcing-locations/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Network optimization recommending facility/sourcing locations</h1>
